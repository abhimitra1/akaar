...
| `009_manager_commissions.sql` | Not yet — superseded by `010`/`011`/`012` below before it's ever applied; run all four together in order. Historical only: this is where `commissions`/`commission_reviews`/`notifications` and the first `is_manager` flag were introduced, before the studio role hierarchy replaced that flag and renamed the tables. |
| `010_role_hierarchy.sql` | Not yet — run this (after `006` and `009`) to replace `is_manager` with the five-tier `profiles.role` hierarchy (`customer`/`artisan`/`designer`/`studio_manager`/`studio_admin`) and the role-assignment authority (`can_assign_role()`) that lets Studio Admins/Managers promote people themselves instead of every promotion routing through a super admin. Also renames `'visitor'` → `'customer'`. |
| `011_rename_commissions_to_orders.sql` | Not yet — run this (after `010`) to rename `commissions`/`commission_reviews` to `orders`/`order_reviews` (and `is_manager()` to `is_studio_manager_or_above()`) throughout. Run before `/studio/orders` or `/orders` will work — the frontend only ever queries the new names. |
| `012_designer_assignment.sql` | Not yet — run this (after `011`) before a Studio Manager can route an order to a Designer, or before `/studio/designer-queue` will show anything. Adds `orders.assigned_designer_id`. |

Run in numeric order where a "Depends on" note in a file's own header says so (006 before
007, 008, 009, and 010; 010 before 011; 011 before 012; otherwise independent). All are
